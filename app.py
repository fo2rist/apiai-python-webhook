...
def processRequest(req):
    ...
    if action == "exit":
        return processExitRequest(result)
    elif action == "yahooWeatherForecast":
        return processWeatherRequest(result)
    else:
        return {}
...
def processWeatherRequest(result):
    yql_query = makeYqlQuery(result)
...
def makeYqlQuery(result):
    ...
    if not us_state is None:
        query_string += " "
        query_string += us_state
    ...
    return "select * from weather.forecast where woeid in (select woeid from geo.places(1) where text='" + query_string + "') and u='c'"
